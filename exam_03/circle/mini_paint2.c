...
int	clear(char *drawing, FILE *file)
{
	fclose(file);
	if (drawing)
		free(drawing);
...
void	draw_term(char *str, t_zone *zone)
{
	int	i;
...
int	main(int argc, char **argv)
{
	...
	if (!(drawing = paint_background(&zone)))
		return (clear(drawing, file) && str_error("Error: malloc failed", 1));
	if (!(draw_shapes(&zone, &drawing, file)))
		return (clear(drawing, file) && str_error("Error: Operation file corrupted", 1));
